...
#Displays the strength of the generated password to users, based on selections for length and characters.
#Allows users to show/hide generated password.
#Uses Secrets module (rather than Random module) to generate password. 
...
password_entry = ctk.CTkEntry(root, border_width=2, placeholder_text="Password", show='*')
...
def print_selection():
    ...
    if password_chars:  # Check if any characters are selected
        generated_password = ''.join(secrets.choice(password_chars) for _ in range(int(get_length())))  # Generate a password of length 12
...
def toggle_password():
    if password_entry.cget('show') == '':  # If password is visible
        password_entry.configure(show='*')  # Hide it
...
toggle_button = ctk.CTkButton(root, text="Show Password", text_color="black", fg_color="red", hover_color="red3", command=toggle_password)
toggle_button.pack()
